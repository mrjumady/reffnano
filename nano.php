...
$colors = new \Colors();
use Curl\Curl;
echo ''.$colors->getColoredString('-------', 'yellow').' Auto Reff Nanovest, Manual Input'.$colors->getColoredString('-------', 'yellow').''.PHP_EOL.PHP_EOL;
echo '['.$colors->getColoredString(date('H:i:s'), 'green').'] | Masukan Reff Nanovest: ';
$reff = trim(fgets(STDIN));
while (-1) {
	echo '---------------------------------------------------------------------'.PHP_EOL;
	echo '['.$colors->getColoredString(date('H:i:s'), 'green').'] | Nomor HP (62): ';
	$getNo = trim(fgets(STDIN));
	$noHP = substr($getNo, 2);
	...
	if($curl->response->code == "1") {
	    ...
	    tryOtp:
	    echo '['.$colors->getColoredString(date('H:i:s'), 'green').'] | Kode OTP: ';
	    $codeOTP = trim(fgets(STDIN));
	    $curl = new Curl();
	    $curl->setUserAgent('okhttp/3.12.12');
		  $curl->setHeader('accept', 'application/json, text/plain, */*');
		  $curl->setHeader('x-device-id', ''.$did.'');
